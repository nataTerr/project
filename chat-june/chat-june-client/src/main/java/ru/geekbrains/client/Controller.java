...
public class Controller {
    ...
    @FXML
    HBox authPanel, msgPanel; //панели для авторизации, ввода сообщений
    ...
    public void auth(ActionEvent event) {
        //метод авторизации и регистрации пользователя
        try {
            Button sourceButton = (Button) event.getSource();
            if (sourceButton.equals(v1)) { //кнопка входа
            ...
            if (sourceButton.equals(v2)) { //кнопка регистрации
                out.writeUTF("/reg " + userName.getText()); //в окне для авторизации пишем /reg имя пользователя введенное клиентом
                userName.clear(); //очищаем поле
            }
        } catch (IOException e) {
    ...
    public void connect() {
        ...
        try {
            socket = new Socket("localhost", 8180); //подключение к серверу
    ...
    private void readMessageFromServerAuth() {
        //авторизация
        try {
            while (true) {
